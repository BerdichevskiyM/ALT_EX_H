...
def test_module_H():
    ...
    def test_ADD_HH_H():
        ...
        h1 = HNum(q1, q2, QNum(z2, n1), QNum(z2, n1))  # 1 + 2i + 2j + 2k
        h2 = HNum(qm1, q1, qm1, q1)  # -1 + 1i - 1j + 1k

        # Ожидаем: (1 + (-1)) + (2 + 1)i + (2 + (-1))j + (2 + 1)k = 0 + 3i + 1j + 3k
        result = ADD_HH_H_f(h1, h2)

        assert result.s.num_tor.A == [0]
        assert result.s.den_tor.A == [1]
    ...
    def test_COS_HH_Q():
        ...
        # cos(0) = 1
        assert q_to_float(cos_val2) == pytest.approx(1, 0.01)
    ...
    def test_MUL_RI_HH_H():
        ...
        assert result.s.num_tor.A == [0]
        assert result.x.num_tor.A == [6]
        assert result.y.num_tor.A == [8]
        assert result.z.num_tor.A == [0, 1]

        result2 = MUL_RI_HH_H_f(h2, h1)
        assert result2.s.num_tor.A == [0]
        assert result2.x.num_tor.A == [0]
    ...
    def test_NEG_H():
        ...
        assert neg2.y.num_tor.A == [3] and neg2.y.num_tor.b == 1
    ...
    def test_NORM_H_Q():
        ...
        assert sqr_norm.num_tor.A == [5, 2]
        assert sqr_norm.den_tor.A == [1]

        # Норма = sqrt(25) = 5
        norm = NORM_H_Q_f(h)
        assert q_to_float(norm) == pytest.approx(5, 0.01)

        h2 = create_h(1, 2, 2, 1)
        sqr_norm2 = SQR_NORM_H_Q_f(h2)
        assert sqr_norm2.num_tor.A == [0, 1]
        assert sqr_norm2.den_tor.A == [1]

        # sqrt(10) ~ 3.16227766...
        norm2 = NORM_H_Q_f(h2)
        assert q_to_float(norm2) == pytest.approx(3.16, 0.01)
    ...
    def test_NORML_H():
        ...
        assert q_to_float(norm_h.x) == pytest.approx(0.6, 0.01)
        assert norm_h.y.num_tor.A == [0]
        assert q_to_float(norm_h.z) == pytest.approx(0.8, 0.01)

        # Норма нормализованного кватерниона должна быть 1
        sqr_norm = SQR_NORM_H_Q_f(norm_h)
        assert q_to_float(sqr_norm) == pytest.approx(1, 0.01)
    ...
    def test_ROT_Q_H():
        ...
        """
        from modules.H.rot_run_tests import run_all
